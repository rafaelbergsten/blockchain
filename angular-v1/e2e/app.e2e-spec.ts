...
describe('Starting tests for angular-v1', function() {
  ...
  it('website title should be angular-v1', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('angular-v1');
    })
  });
  ...
  it('navbar-brand should be angular-v1',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('angular-v1');
    });
  });
