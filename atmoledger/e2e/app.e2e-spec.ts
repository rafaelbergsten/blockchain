...
describe('Starting tests for atmoledger', function() {
  ...
  it('website title should be atmoledger', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('atmoledger');
    })
  });
  ...
  it('navbar-brand should be atmoledger',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('atmoledger');
    });
  });
