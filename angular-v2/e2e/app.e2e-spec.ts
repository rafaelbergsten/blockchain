...
describe('Starting tests for angular-v2', function() {
  ...
  it('website title should be angular-v2', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('angular-v2');
    })
  });
  ...
  it('navbar-brand should be angular-v2',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('angular-v2');
    });
  });
