...
describe('Starting tests for angular-v3', function() {
  ...
  it('website title should be angular-v3', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('angular-v3');
    })
  });
  ...
  it('navbar-brand should be angular-v3',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('angular-v3');
    });
  });
